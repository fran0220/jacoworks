...
func (h *WSHandler) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		log.Warn().Err(err).Str("user_id", userID).Msg("openclaw ws bridge: upgrade failed")
		return
	}
	defer conn.Close()
	...
	log.Info().
		...
		Msg("openclaw ws bridge: client connected")
	...
	log.Info().
		Str("user_id", userID).
		Str("container", containerName).
		Msg("openclaw ws bridge: client disconnected")
}
...
func (h *WSHandler) runConnection(conn *websocket.Conn, channel *UserChannel, userID string, replay []SSEEvent, updates <-chan SSEEvent) {
	...
	enqueue := func(payload []byte) bool {
		...
		case <-done:
			return false
		default:
			log.Warn().Str("user_id", userID).Int("queue_size", len(outbound)).Msg("openclaw ws bridge: outbound queue overflow")
			closeConn()
			return false
		}
	}
	...
	for _, event := range replay {
		...
		if err != nil {
			log.Debug().Err(err).Str("user_id", userID).Msg("openclaw ws bridge: skip invalid replay event")
			continue
		}
		if !enqueue(payload) {
			<-done
	...
	if status := channel.Status(); status.Connected {
		...
		payload, err := marshalWSEventFrame(SSEEvent{Event: "proxy.ready", Data: readyPayload})
		if err == nil {
			if !enqueue(payload) {
				<-done
				return
	...
	go func() {
		...
		for {
			...
			case event, ok := <-updates:
				...
				payload, err := marshalWSEventFrame(event)
				if err != nil {
					log.Debug().Err(err).Str("user_id", userID).Msg("openclaw ws bridge: skip invalid update event")
					continue
				}
				if !enqueue(payload) {
					return
...
func (h *WSHandler) handleClientMessage(channel *UserChannel, payload []byte, enqueue func([]byte) bool) (bool, error) {
	...
	case "req":
		if strings.TrimSpace(frame.Method) == "" {
			enqueueProxyError(enqueue, "method is required")
			return true, fmt.Errorf("method is required")
		}

		err := channel.SendRequest(frame.Method, frame.Params, frame.ID)
		if err != nil {
			if errors.Is(err, ErrChannelReconnecting) || errors.Is(err, ErrChannelNotConnected) {
				enqueueProxyError(enqueue, "openclaw reconnecting")
				return true, err
			}
		...
		return true, nil
	default:
		enqueueProxyError(enqueue, "unsupported message type")
		return false, fmt.Errorf("unsupported message type: %s", frame.Type)
	}
}
...
func marshalWSEventFrame(event SSEEvent) ([]byte, error) {
	data := event.Data
