...
**根因**:
1. 我先根据仓库里的 fallback 常量推断默认值，没有先核对线上 `system_settings` / `/api/agent/config` 的真实返回。
2. 桌面端把网关模型列表塞进模块级变量，组件用 `getModelOptions()` 直接读取，虽然值会变，但不是 React state，界面更新不可依赖这种模式。

**修复原则**:
- 涉及默认配置、线上行为、系统设置时，先查 live DB 或真实接口，再决定改动方向。
- 前端只要某份数据会影响渲染，就必须进入 React state / props / context，禁止再用模块级可变单例承载 UI 数据源。

**规则**:
- 修改 `primary_model`、模型列表、开关默认值前，先验证线上真实值，不能只看本地 seed/fallback。
- UI 配置源如果来自网关，下游组件必须消费 React state，不能靠“先写全局，再调用 getter”这种隐式共享。
