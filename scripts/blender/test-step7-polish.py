#!/usr/bin/env python3
"""
Step 7: Polish — density/balance pass on step6 layout.
Fixes:
  #1: Pull all perimeter furniture inward (max ±9.8) to prevent wall clipping
  #2: Hub enriched with low bookcase, layered rugs, more plants, bear mascot
  #3: More green — plants along corridors, varied rug colors
  #4: Corridor decoration — benches, plants, coat racks along outer walls
  #5: Corner alignment verification
  #6: Remove _tmp sideTable hack, use constant SIDE_TABLE_TOP_Z

Usage:
    /Applications/Blender.app/Contents/MacOS/Blender --background --factory-startup \
        --python scripts/blender/test-step7-polish.py
"""
import bpy
import os
import math
...
CEILING_Z = 1.29 * S - 0.05  # just below wall top
...
def validate_scene():
    ...
    for name, (ex, ey) in expected_corners.items():
        ...
        if o:
            if abs(o.location.x - ex) > 0.01 or abs(o.location.y - ey) > 0.01:
                print(f"  ⚠️  Corner misaligned: {name} @ ({o.location.x:.2f}, {o.location.y:.2f}) expected ({ex}, {ey})")
            else:
                print(f"  ✅ {name} aligned")

    skip_prefixes = ("f_", "w_", "wd_", "ww_", "ph_", "pv_", "corner_", "door_", "wl_", "al_")
    violations = 0
    for o in bpy.data.objects:
        ...
        if mnx < -SAFE_LIMIT - 1 or mxx > SAFE_LIMIT + 1 or mny < -SAFE_LIMIT - 1 or mxy > SAFE_LIMIT + 1:
            print(f"  ⚠️  Out of bounds: {o.name} x[{mnx:.1f},{mxx:.1f}] y[{mny:.1f},{mxy:.1f}]")
            violations += 1
    if violations == 0:
        print("  ✅ All furniture within bounds")


# ══════════════════════════════════════════════════════════
print("\n" + "="*60)
print("  Step 7: Polish (density + balance pass)")
print("="*60)
...
# FIX #1: Pull perimeter props inward (was ±4.5 from center = ±10.5 absolute)
...
# Micro-detail: cardboard boxes near outer wall
...
# FIX #1: Tower bookcase pulled inward (was -10.5, 10.5)
...
# FIX #1: Court props pulled inward
...
# FIX #1: Kitchen corner pulled inward (was x+4.5 = 10.5)
...
# Micro-detail: toaster on counter
...
imp("rugRound.glb", pos=(lx+3, ly+3, 0.01), sc=S*0.8, name="lg_krug")
imp("plantSmall2.glb", pos=(lx+3.4, ly+1, cnt_z), name="lg_splant")

# ── HUB — 大厅 (0, 0) (FIX #2: enriched) ─────────────────
print("\n🏛 Hub...")
...
# FIX #2: Layered rugs for visual anchor
imp("rugRounded.glb", pos=(0, -2, 0.005), sc=S*2.2, name="hub_rug_outer")
imp("rugRound.glb", pos=(0, -2, 0.01), sc=S*1.8, name="hub_rug")
imp("rugDoormat.glb", pos=(0, 1.3, 0.01), name="hub_deskmat")

# FIX #2: Low display shelf behind reception (keeps sightlines open)
imp("bookcaseOpenLow.glb", pos=(0, 2.8, 0), name="hub_shelf")

# FIX #2: More plants for visual weight
imp("pottedPlant.glb", pos=(-3.5, 3.5, 0), sc=S*1.3, name="hub_p1")
imp("pottedPlant.glb", pos=(3.5, 3.5, 0), sc=S*1.3, name="hub_p2")
imp("pottedPlant.glb", pos=(-2.8, 1.0, 0), name="hub_p3")
imp("pottedPlant.glb", pos=(2.8, 1.0, 0), name="hub_p4")
...
# FIX #6: Use constant instead of _tmp hack
...
# ── CORRIDOR DECORATION (FIX #3 & #4) ────────────────────
print("\n🌿 Corridor decoration...")

# North corridor (outer wall y≈12, between partitions and wall)
...
# East corridor (near entrance doors)
...
out = os.path.join(PREVIEW_DIR, "step7-polish.png")
