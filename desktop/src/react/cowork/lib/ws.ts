...
export interface CoworkWSHandlers {
  onReady?: () => void;
...
export class CoworkWS {
  ...
  constructor(private handlers: CoworkWSHandlers = {}) {}
  ...
  private async openSocket() {
    ...
    try {
      ...
      socket.onerror = () => {
        if (!this.isActiveSocket(socket, runId)) return;
        this.handlers.onError?.(new Error("协作 WebSocket 连接错误"));
      };
  ...
  private async fetchTicket(token: string) {
    ...
    if (response.status !== 200) {
      throw new Error(parseHttpError(response.body, `获取协作 WS ticket 失败 (${response.status})`));
    }

    const body = parseJson<WsTicketResponse>(response.body);
    if (!body?.ticket) {
      throw new Error(body?.error || body?.message || "获取协作 WS ticket 失败");
    }
  ...
  private handleIncomingText(raw: string) {
    ...
    switch (event) {
      ...
      case "proxy.error":
        this.handlers.onError?.(new Error(normalizeErrorMessage(data, "协作代理错误")));
        break;
    }
  }
