...
export async function updateOcSession(id: string, data: Partial<OcSession>) {
  ...
  if (response.status !== 200) {
    throw new Error(parseError(response.body, `更新协作会话失败 (${response.status})`));
  }
}
...
export async function deleteOcSession(id: string) {
  ...
  if (response.status !== 204) {
    throw new Error(parseError(response.body, `删除协作会话失败 (${response.status})`));
  }
}
